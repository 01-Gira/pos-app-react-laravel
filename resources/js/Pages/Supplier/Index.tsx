...
export default function Index({ title, auth, suppliers, pagination, search, flash }: PageProps) {
    ...
    const onPageChange = (page: number) => {
        router.get(route('master.suppliers.index'), { search: searchQuery, page }, { preserveState: true });
        setCurrentPage(page);
    };

    const onRowsPerPageChange = (newRowsPerPage: number, page: number) => {
        setRowsPerPage(newRowsPerPage);
        router.get(route('master.suppliers.index'), { search: searchQuery, page, per_page: newRowsPerPage }, { preserveState: true });
        setCurrentPage(page);
    };

    const onSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setSearchQuery(event.target.value);
        router.get(route('master.suppliers.index'), { search: event.target.value, page: 1 }, { preserveState: true });
    };
    ...
    const columns : TableColumn<Supplier>[]= [
        ...
        {
            name: 'Created At',
            selector: (row : Supplier)=> format(new Date(row.created_at), 'yyyy-MM-dd HH:mm:ss'),
            sortable: true,
        },
        {
            name: 'Updated At',
            selector: (row: Supplier) => format(new Date(row.updated_at), 'yyyy-MM-dd HH:mm:ss'),
            sortable: true,
        },
        {
            name: 'Action',
        ...
        >
            ...
            <div className='p-7 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg'>
                ...
                <div className="flex justify-between items-center space-x-2">
                    <Button href={route('master.suppliers.create')} className="w-40 hover:bg-cyan-800">
