...
import { PageProps,Product,ProductTransactions, Transaction } from "@/types";
...
export default function Index({ title, auth, flash }: PageProps) {
    ...
    const newTransaction = async () => {
        ...
        const res = await axios.post(route('transaction.cashier.new-transaction'));
                    ...
                    setTransactions((prevTransactions) => {
                        const updatedTransactions = prevTransactions.map(transaction => {
                            if (transaction.id === currentTransactionId) {
                                const existingProductIndex = transaction.products?.findIndex(p => p.id === product.id);

                                if (existingProductIndex !== undefined && existingProductIndex !== -1 && transaction.products) {
                                    const updatedProducts = [...transaction.products];
                                    updatedProducts[existingProductIndex].quantity += 1;
                                    updatedProducts[existingProductIndex].total_price = calculateTotalPrice(
                                        updatedProducts[existingProductIndex].price,
                                        updatedProducts[existingProductIndex].quantity,
                                        updatedProducts[existingProductIndex].discount
                                    );
                                    return { ...transaction, products: updatedProducts };
                                } else {
                                    const newProduct = {
                                        ...product,
                                        quantity: 1,
                                        total_price: calculateTotalPrice(product.price, 1, product.discount)
                                    };
                                    return {
                                        ...transaction,
    ...
    const saveTransaction = async ( transactions: Transaction[]) => {
        try {
            setLoading(true);
            const res = await axios.post(route('transaction.cashier.scan-product'), {
                transactions
            });
            setLoading(false);
        ...
        .then(async(conf) => {
            if (conf) {
                if (transactions && currentTransactionId) {
                    const res = await axios.put(route("transaction.cashier.hold-transaction", currentTransactionId));

                    if(res.data.type_message == 'success'){
                        window.location.reload();
                    }
                } else {
                    swal("Sorry, you cannot hold this transaction because there are transaction on going.");
                }
    ...
    const submitTransaction = async () => {
        ...
        if(transactionID != ''){
            try {
                setLoading(true);
                const res = await axios.post(route('transaction.cashier.submit-transaction', {transactionID}), {
                    transactions
                });
                console.log(res);
            } catch (error) {
    ...
    const calculateTotalPrice = (price: number, quantity: number, discount: number) => {
    ...
    const currentTransaction = transactions.find(transaction => transaction.id === currentTransactionId);


    const totalPrice = useMemo(() => {
        const total = currentTransaction?.products?.reduce((total, product) => {
    ...
    const totalPayment = useMemo(() => Math.round(totalPrice + totalPPN), [totalPrice]);
    ...
    useEffect(() => {
        if (currentTransaction) {
            ...
            if (spanTotalPrice.current) {
                spanTotalPrice.current.textContent = formatRupiah(currentTransaction.total_price);
            }
            if (spanTotalPPN.current) {
                spanTotalPPN.current.textContent = formatRupiah(currentTransaction.ppn);
            }
            if (spanTotalPayment.current) {
                spanTotalPayment.current.textContent = formatRupiah(totalPayment);
            }
    ...
    const listHoldTransaction = async() => {
        const res = await axios.get(route("transaction.cashier.get-data-transactions"));
        // console.log(res);
        // setOpenModal(true);
    }
                    ...
                    <div className="flex justify-between">
                        ...
                        <div className="flex">
                            {loading ? (
                                ...
                                <>
                                    <Button onClick={() => listHoldTransaction()}>List Hold Transaction</Button>
                                    <Button className="ms-3" ref={buttonTransaction} onClick={() => newTransaction()}>New Transaction</Button>
                                </>
                            )}
                        </div>
                    </div>
                    <Modal show={openModal} onClose={() => setOpenModal(false)} className="w-full h-full">
                        <Modal.Header>
                        ...
                        {transactions.map(transaction => (
                                transaction.products?.map((product: Product, index) => (
                                    <Table.Row key={product.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                        ...
                                        <Table.Cell>{product.category?.category_name}</Table.Cell>
                                        <Table.Cell>
                                            <TextInput
                                                type="number"
                                                ...
                                                onChange={async (e) => {
                                                    ...
                                                    updatedProducts[index].total_price = calculateTotalPrice(
                                                    ...
                                                    setTransactions(prevTransactions => prevTransactions.map(t => {
                                                        if (t.id === transaction.id) {
                                                            return { ...t, products: updatedProducts };
                                                        }
                                                        return t;
                                                    }));
                                        ...
                                        <Table.Cell>{formatRupiah(product.price)}</Table.Cell>
                                        <Table.Cell>
                                            {loading ? (
                                                <BeatLoader />
                                            ) : (
                                                <TextInput
                                                    ...
                                                    onChange={async (e) => {
                                                        const updatedProducts = [...(transaction.products || [])];
                                                        updatedProducts[index].discount = parseInt(e.target.value);
                                                        updatedProducts[index].total_price = calculateTotalPrice(
                                                            updatedProducts[index].price,
                                                            updatedProducts[index].quantity,
                                                            updatedProducts[index].discount
                                        ...
                                        <Table.Cell>{formatRupiah(product.total_price) || 0}</Table.Cell>
                                    </Table.Row>
                                ))
                            ))}
                        </Table.Body>
                    ...
                    <div className="mt-5 flex justify-between">
                        ...
                        <div className="flex">
                            {loading ? (
                                ...
                                <>
                                 <Button color="red" onClick={() => holdTransaction()}>Hold</Button>
                                 <Button color="green" className="ms-3" onClick={() => submitTransaction()}>Submit</Button>
                                </>
                            )}
