...
export default function Index({ title, auth, discounts, pagination, search, flash }: PageProps) {
    ...
    const onPageChange = (page: number) => {
        router.get(route('master.discounts.index'), { search: searchQuery, page }, { preserveState: true });
        setCurrentPage(page);
    };

    const onRowsPerPageChange = (newRowsPerPage: number, page: number) => {
        setRowsPerPage(newRowsPerPage);
        router.get(route('master.discounts.index'), { search: searchQuery, page, per_page: newRowsPerPage }, { preserveState: true });
        setCurrentPage(page);
    };

    const onSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setSearchQuery(event.target.value);
        router.get(route('master.discounts.index'), { search: event.target.value, page: 1 }, { preserveState: true });
    };
    ...
    const columns : TableColumn<Discount>[]= [
        ...
        {
            name: 'Created At',
            selector: (row : Discount)=> format(new Date(row.created_at), 'yyyy-MM-dd HH:mm:ss'),
            sortable: true,
        },
        {
            name: 'Updated At',
            selector: (row: Discount) => format(new Date(row.updated_at), 'yyyy-MM-dd HH:mm:ss'),
            sortable: true,
        },
        {
            name: 'Action',
